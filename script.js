...
const API_BASE_URL = (() => {
    ...
    // Fallback for file:// access or environments without origin
...
// Global modal references (cached on page load)
let notificationsModalElement = null;
let compareModalElement = null;
let priceHistoryModalElement = null;
...
// Initialize buttons to be visible and functional
function initializeButtons() {
    ...
    if (notificationsBtn) {
        ...
        console.log('Notifications button initialized');
    }
    ...
    // Cache modal references globally
    notificationsModalElement = document.getElementById('notificationsModal');
    compareModalElement = document.getElementById('compareModal');
    priceHistoryModalElement = document.getElementById('priceHistoryModal');
    
    console.log('Modal verification:');
    console.log('- notificationsModal:', notificationsModalElement ? 'EXISTS ✓' : 'MISSING ✗');
    console.log('- compareModal:', compareModalElement ? 'EXISTS ✓' : 'MISSING ✗');
    console.log('- priceHistoryModal:', priceHistoryModalElement ? 'EXISTS ✓' : 'MISSING ✗');
    ...
    fetch(`${API_BASE_URL}/offers`)
        ...
        .then(data => {
            if (data.success) {
                ...
                // Populate filters from the offers data directly
                populateFiltersFromOffers(allOffers);
...
// Populate filters from offers data
function populateFiltersFromOffers(offers) {
    if (!offers || !Array.isArray(offers)) {
        console.error('❌ Invalid offers data for filter population');
        return;
    }
    ...
    grid.innerHTML = offers.map(offer => {
        ...
        <div class="offer-card">
            ...
            <div class="offer-image">
                <img src="${offer.image || 'https://via.placeholder.com/300x200?text=Product'}" alt="${offer.title}" onerror="this.src='https://via.placeholder.com/300x200?text=Product'" loading="lazy">
            </div>
...
async function applyFilters() {
    ...
    if (!categoryEl || !platformEl || !discountEl || !searchEl) {
        console.error('❌ Filter elements not found');
        showError('Filter controls not available');
        return;
    }
    
    const category = categoryEl.value;
    const platform = platformEl.value;
    const minDiscount = discountEl.value;
    const searchQuery = searchEl.value.trim();
    
    console.log('📊 Filter values:', { category, platform, minDiscount, searchQuery });
    ...
    try {
        ...
        const url = `${API_BASE_URL}/offers?${params}`;
        console.log('🌐 Fetching filtered offers from:', url);
        ...
        if (data.success) {
            ...
            hideLoading();
            console.log(`✅ Displayed ${data.offers.length} filtered offers`);
        } else {
            console.error('❌ Filter API returned error:', data);
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="alert-product">
                    <img src="${offer.image || 'https://via.placeholder.com/200x200?text=Product'}" alt="${offer.title}">
...
function showNotifications() {
    ...
    // Use cached reference first, fallback to DOM query
    const modal = notificationsModalElement || document.getElementById('notificationsModal');
    ...
    if (modal) {
        ...
        // Try to re-cache the modal
        notificationsModalElement = document.getElementById('notificationsModal');
        if (notificationsModalElement) {
            notificationsModalElement.classList.add('active');
            renderNotifications();
        }
    }
}
...
function updateCompareButtons() {
    ...
    document.querySelectorAll('.compare-btn').forEach(btn => {
        ...
        const match = onclickAttr ? onclickAttr.match(/toggleCompare\((\d+)\)/) : null;
...
function showCompare() {
    console.log('showCompare() called');
    // Use cached reference first, fallback to DOM query
    const modal = compareModalElement || document.getElementById('compareModal');
    console.log('Compare modal element:', modal);
    if (modal) {
        modal.classList.add('active');
        console.log('Modal classes after adding active:', modal.className);
        renderCompareList();
        ...
        if (compareModalElement) {
            compareModalElement.classList.add('active');
            renderCompareList();
        }
    }
}
            ...
            <div class="compare-row header">
                ...
                ${compareList.map(offer => `
                    <div class="product-cell">
                        <img src="${offer.image || 'https://via.placeholder.com/60?text=Product'}" alt="${offer.title}" onerror="this.src='https://via.placeholder.com/60?text=Product'">
                        <h4>${offer.title}</h4>
                    </div>
                `).join('')}
            </div>
...
// Global CNN matcher instance
let cnnMatcher = null;

// Initialize CNN matcher when page loads
// Visual Search functionality - Make globally available
function openVisualSearch() {
    console.log('🔍 Opening Visual Search...');
    try {
        // Try opening in new tab first
        let newWindow = window.open('visual-search.html', '_blank');
        
        if (newWindow) {
            console.log('✅ Visual Search opened successfully');
            newWindow.focus();
        } else {
            // Fallback: try opening in same window
            console.log('🔄 Trying fallback method...');
            window.location.href = 'visual-search.html';
        }
    } catch (error) {
        console.error('❌ Error opening Visual Search:', error);
        // Ultimate fallback: show user how to access it
        alert('Visual Search available at: visual-search.html\n\nOr allow popups for this site and try again.');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    visualSearchScript.src = 'visual-search.js';
    document.head.appendChild(visualSearchScript);
    
    // Make sure the function is globally available
    window.openVisualSearch = openVisualSearch;
    console.log('✅ Visual Search function loaded and available');
    
    // Add backup click handler for visual search button
    const visualSearchBtn = document.querySelector('.visual-search-btn');
    if (visualSearchBtn) {
        visualSearchBtn.addEventListener('click', function(e) {
            console.log('🔍 Visual Search button clicked via event listener');
            openVisualSearch();
        });
        console.log('✅ Visual Search button event listener attached');
    } else {
        console.error('❌ Visual Search button not found');
    }
});

async function findSimilarProducts(imageSrc) {
    if (!cnnMatcher) {
        console.warn('⚠️ CNN matcher not initialized, using fallback');
        return fallbackSimpleMatching(imageSrc);
    }
    
    try {
        console.log('🧠 Using Deep Learning CNN-based Image Matching...');
        
        const similarProducts = await cnnMatcher.findSimilarProducts(imageSrc);
        
        console.log(`📊 CNN found ${similarProducts.length} similar products`);
...
function fallbackSimpleMatching(imageSrc) {
...
function displayImageSearchResults(products) {
    ...
    resultsDiv.innerHTML = products.map(offer => `
        <div class="offer-card similar-product" onclick="openOffer('${offer.link}')">
            ...
            <div class="offer-image">
                <img src="${offer.image || 'https://via.placeholder.com/300x200?text=Product'}" alt="${offer.title}" onerror="this.src='https://via.placeholder.com/300x200?text=Product'">
            </div>
            <div class="offer-content">
                <h3>${offer.title}</h3>
                <div class="offer-price">
...
function initVoiceRecognition() {
    ...
    if (!SpeechRecognition) {
        ...
        updateVoiceStatus('Voice recognition not supported in your browser. Try Chrome or Edge.', 'error');
        
        // Show fallback text input
        showVoiceSearchFallback();
        return;
    }
    ...
    try {
        ...
        updateVoiceStatus('Failed to initialize voice recognition', 'error');
        showVoiceSearchFallback();
    }
}
...
function showVoiceSearchFallback() {
    const voiceStatus = document.getElementById('voiceStatus');
    voiceStatus.innerHTML = `
        <div class="voice-fallback">
            <p>🎤 Voice search not available. Use text search instead:</p>
            <input type="text" id="voiceFallbackInput" placeholder="Type what you're looking for..." 
                   style="width: 100%; padding: 10px; margin: 10px 0; border: 2px solid #007bff; border-radius: 5px;">
            <button onclick="processVoiceFallback()" class="btn btn-primary">Search</button>
        </div>
    `;
...
function processVoiceFallback() {
    const input = document.getElementById('voiceFallbackInput');
    if (!input) return;
    
    const transcript = input.value.trim();
    if (!transcript) {
        updateVoiceStatus('Please enter what you\'re looking for', 'error');
        return;
    }
    
    // Show the transcript as if it was voice input
    document.getElementById('voiceTranscript').style.display = 'block';
    document.getElementById('transcriptText').textContent = transcript;
...
// Setup voice circle click handler and search input
document.addEventListener('DOMContentLoaded', () => {
...
function showPriceHistory(offerId) {
    console.log('showPriceHistory() called for offer ID:', offerId);
    const offer = allOffers.find(o => o.id === offerId);
    if (!offer) {
    ...
    const priceHistoryImage = document.getElementById('priceHistoryImage');
    const priceHistoryTitle = document.getElementById('priceHistoryTitle');
    const priceHistoryDescription = document.getElementById('priceHistoryDescription');
    const currentPrice = document.getElementById('currentPrice');
    
    if (priceHistoryImage) {
        priceHistoryImage.src = offer.image || 'https://via.placeholder.com/300x200?text=Product';
    } else {
        console.error('priceHistoryImage element not found');
    }
    
    if (priceHistoryTitle) {
        priceHistoryTitle.textContent = offer.title;
    } else {
        console.error('priceHistoryTitle element not found');
    }
    
    if (priceHistoryDescription) {
        priceHistoryDescription.textContent = offer.description;
    } else {
        console.error('priceHistoryDescription element not found');
    }
    
    if (currentPrice) {
        currentPrice.textContent = `₹${formatNumber(offer.discountedPrice)}`;
    } else {
    ...
    if (priceChangeEl) {
        ...
        console.error('priceChange element not found');
    }
    ...
    if (trackBtn) {
        ...
        console.error('trackPriceBtn element not found');
    }
    ...
    // Show modal
    const modal = priceHistoryModalElement || document.getElementById('priceHistoryModal');
    if (modal) {
        modal.classList.add('active');
        console.log('Price history modal opened successfully');
    } else {
        console.error('priceHistoryModal not found!');
        // Try to re-cache the modal
        priceHistoryModalElement = document.getElementById('priceHistoryModal');
        if (priceHistoryModalElement) {
            priceHistoryModalElement.classList.add('active');
        }
    }
}
